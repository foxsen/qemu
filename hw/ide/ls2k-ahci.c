 ...
 * 6 port SATA AHCI Controller [8086:2922] (rev 02) (prog-if 01 [AHCI 1.0])
 *         Subsystem: Intel Corporation 82801IR/IO/IH (ICH9R/DO/DH) 6 port SATA
 *         AHCI Controller [8086:2922]
...
static const VMStateDescription vmstate_ls_ahci = {
        .name = "ls2k_ahci",
        .version_id = 1,
        .fields = (VMStateField[])
        {
...
static void pci_ls_reset(DeviceState *dev)
{
...
static void pci_ls_ahci_init(Object *obj)
{
        struct AHCIPCIState *d = LS2K_AHCI(obj);

        ahci_init(&d->ahci, DEVICE(obj));
}

static void pci_ls_ahci_realize(PCIDevice *dev, Error **errp)
{
        ...
        ICH9_SATA_CAP_OFFSET, SATA_CAP_SIZE, errp);
...
static void pci_ls_uninit(PCIDevice *dev)
{
        struct AHCIPCIState *d;
        d = LS2K_AHCI(dev);
...
static void ls_ahci_class_init(ObjectClass *klass, void *data)
{
        DeviceClass *dc = DEVICE_CLASS(klass);
        PCIDeviceClass *k = PCI_DEVICE_CLASS(klass);

        k->realize = pci_ls_ahci_realize;
        k->exit = pci_ls_uninit;
        k->vendor_id = 0x14;
        k->device_id = 0x7a08;
        k->revision = 0x02;
        k->class_id = PCI_CLASS_STORAGE_SATA;
        dc->vmsd = &vmstate_ls_ahci;
        dc->reset = pci_ls_reset;
        set_bit(DEVICE_CATEGORY_STORAGE, dc->categories);
}

static const TypeInfo ls_ahci_info = {
        .name          = "ls2k-ahci",
        .parent        = TYPE_PCI_DEVICE,
        .instance_size = sizeof(AHCIPCIState),
        .instance_init = pci_ls_ahci_init,
        .class_init    = ls_ahci_class_init,
...
static void ls_ahci_register_types(void)
{
        type_register_static(&ls_ahci_info);
}

type_init(ls_ahci_register_types)
